...
export default function InvoiceDetailPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {shareUrl && (
        <div className="mb-4 rounded-lg bg-indigo-50 border border-indigo-200 p-4 flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-indigo-900">Shareable invoice link:</p>
            <p className="text-sm text-indigo-700 mt-1 break-all">{shareUrl}</p>
          </div>
          <button onClick={handleCopyLink} className="ml-4 px-3 py-1 rounded bg-indigo-600 text-white text-sm hover:bg-indigo-500">
            {copied ? 'Copied!' : 'Copy'}
          </button>
        </div>
      )}
