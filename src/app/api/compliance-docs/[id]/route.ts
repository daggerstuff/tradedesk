...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await query(
    ...
    [body.doc_name, body.doc_type, body.customer_id || null, body.expiry_date, body.notes || null, body.status || "active", id, session.userId]
  )
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await query(`DELETE FROM compliance_docs WHERE id = $1 AND user_id = $2`, [id, session.userId])
  return NextResponse.json({ ok: true })
}
