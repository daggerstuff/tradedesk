...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await query(
    ...
    [body.title, body.description || null, body.status, body.scheduled_date || null, body.location || null,
     body.estimate_amount || null, body.final_amount || null, body.notes || null, id, session.user.id]
  )
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await query(`DELETE FROM jobs WHERE id = $1 AND user_id = $2`, [id, session.user.id])
  return NextResponse.json({ ok: true })
}
